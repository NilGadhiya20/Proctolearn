...
const emitNotificationToAudience = (notification, audience) => {
...
export const fetchNotificationsForUser = async({ user, limit = 50 }) => {
    const query = {
        ...
        'dismissedBy.user': { $ne: user._id },
        'deletedBy.user': { $ne: user._id }
    };
...
export const clearAllNotificationsForUser = async({ user }) => {
    const query = {
        $or: [
            { 'audience.users': user._id },
            {
    ...
    await Notification.updateMany(query, {
        $addToSet: { deletedBy: { user: user._id, deletedAt: new Date() } }
    });
};

export const upsertAlertNotification = async({
    title,
    message,
    severity,
    audience,
    ...
    const update = {
        $set: {
            ...
            context: {
                ...context,
                metadata: {...(context.metadata || {}), activity: activityKey, count }
            }
        },
    ...
    emitNotificationToAudience(notification, audience);
    return notification;
};
