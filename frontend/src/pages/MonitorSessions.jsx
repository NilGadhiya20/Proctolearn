...
const MonitorSessions = () => {
  ...
  const joinedQuizIdsRef = useRef(new Set());
  ...
  const loadSessions = useCallback(async (options = {}) => {
    ...
    try {
      ...
      // Join quiz rooms to receive live alerts per quiz
      const uniqueQuizIds = new Set(quizzes.map((q) => q._id).filter(Boolean));
      uniqueQuizIds.forEach((quizId) => {
        if (quizId && !joinedQuizIdsRef.current.has(quizId)) {
          socket.emit("joinQuiz", { quizId });
        }
      });
      joinedQuizIdsRef.current = uniqueQuizIds;
      ...
      const inProgress = submissionResponses.flatMap(({ quiz, submissions }) =>
        submissions
          ...
          .map((s) => ({
            ...
            flags:
              (s.suspiciousActivityDetected ? 1 : 0) +
              (s.activityLogs?.length || 0),
            progress: Math.min(
              100,
              Math.round(
                ((s.answers?.length || 0) /
                  Math.max(quiz.totalQuestions || 1, 1)) *
                  100,
              ),
            ),
            timeLeft: "-",
  ...
  useEffect(() => {
    ...
    const pushAlert = (data) => {
      const countLabel = data.count && data.count > 1 ? ` (${data.count})` : "";
      setLiveAlerts((prev) =>
        [
          {
            id: `${Date.now()}-${Math.random()}`,
            alertType: data.alertType || data.activity || "ALERT",
            message: `${data.message || `${data.activity || "Violation"} detected`}${countLabel}`,
    ...
    socket.on("activity-logged", pushAlert);
    socket.on("submission-complete", (data) => {
      // Remove finished session quickly from active list
      setActiveSessions((prev) => prev.filter((s) => s.id !== data.submissionId));
    });

    return () => {
      socket.off("alert", pushAlert);
      socket.off("activity-logged", pushAlert);
      socket.off("submission-complete");
    };
  }, [user]);
              ...
              >
                ...
                <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                  <IconButton
                    ...
                    onClick={() => loadSessions({ showSpinner: true })}
                    sx={{
                      border: "1px solid",
                      borderColor: "divider",
                      borderRadius: 2,
                  ...
                  >
                    <RefreshIcon color={isRefreshing ? "primary" : "action"} />
                  </IconButton>
                  <Button
                    variant="outlined"
                      ...
                      <React.Fragment key={session.id}>
                        <ListItem
                          sx={{
                            ...
                            mb: 1,
                            bgcolor:
                              session.flags > 0 ? "#fff3e0" : "transparent",
                          ...
                          secondaryAction={
                            <Button
                              ...
                              onClick={() =>
                                navigate(`/monitoring/${session.quizId}`)
                              }
                              sx={{
                                textTransform: "none",
                                minHeight: "44px",
                            ...
                            secondary={
                              <Box>
                                ...
                                <Box sx={{ display: "flex", gap: 2, mt: 1 }}>
                                  <Chip
                                    label={`Progress: ${session.progress}%`}
                                    size="small"
                                    variant="outlined"
                                  />
                                  <Chip
                                    label={`Time Left: ${session.timeLeft}`}
                                    size="small"
                                    variant="outlined"
                                  />
                                </Box>
